               ...
               thread_func *function, void *aux) 
{
  ...
  /* Preempt if new thread's priority is higher than current's */
...
{
  if((thread_current()->priority = thread_current()->priority) > (thread_current()->priority = new_priority))
      thread_yield();
}
...
bool prio_less_func (const struct list_elem *pre, const struct list_elem *post, void *aux UNUSED) {
    return list_entry(pre, struct thread, elem)->priority > list_entry(post, struct thread, elem)->priority;
}

#ifdef USERPROG
/* Returns the child thread CHILD_TID. */
struct thread *thread_child (tid_t child_tid) {
    struct list_elem *thread_elem;

    for(thread_elem = list_begin(&(thread_current()->child_list));
            thread_elem != list_end(&(thread_current()->child_list));
